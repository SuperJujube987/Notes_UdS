...
\titleformat{\chapter}[block]{\bfseries\huge\centering}{Chapitre \arabic{chapter}}{1em}{}[]
...
\newenvironment*{subsubproof}[1][D\'emonstration]
{
\begin{proof}[#1]
}
{
...
\begin{document}
	...
	\begin{tikzpicture}
		...
		\node[font=\Large] () at (-.2,-1) {$a$};
		\node[font=\Large] () at (-.7,-2.5) {$b$};
		\node[font=\Large] () at (.4,-3.4) {$c$};
	...
	\phantomsection\section*{Mani\`eres de d\'efinir une fonction}\addcontentsline{toc}{section}{Mani\`eres de d\'efinir une fonction}
	...
	\begin{prop}
		...
		\end{proof}
	\end{prop}
	\chapter[Groupes]{Groupes, chapitre 6 dans le livre de r\'ef\'erence, p.103}
	\begin{defin}
		~
	...
	\phantomsection\section*{Propri\'et\'es \'el\'ementaires des groupes}\addcontentsline{toc}{section}{Propri\'et\'es \'el\'ementaires des groupes}
	\begin{nlist}
	...
	\phantomsection\section*{Produit cart\'esien de groupes}\addcontentsline{toc}{section}{Produit cart\'esien de groupes}
	...
	\begin{exem}
		...
		\end{ulist}
	\end{exem}
	\phantomsection\section*{Isomorphismes de groupes}\addcontentsline{toc}{section}{Isomorphismes de groupes}
	\begin{defin}
		$(G,*)$ et $(H,\diamond)$ deux groupes.
	...
	\phantomsection\section*{Puissances d'\'el\'ements de groupes}\addcontentsline{toc}{section}{Puissances d'\'el\'ements de groupes}
	...
	\phantomsection\section*{Sous-groupes}\addcontentsline{toc}{section}{Sous-groupes}
	\begin{defin}
	...
	\chapter[Applications et \'equivalences]{Applications et \'equivalences, chapitre 2 dans le livre de r\'ef\'erence, p.17}
	\section[Relations d'\'equivalence]{Relations d'\'equivalence, section 4 dans le livre de r\'ef\'erence, p.32}
	\begin{defin}
	...
	\chapter[Groupes (suite)]{Groupes (suite), chapitre 6 dans le livre de r\'ef\'erence, p.103}
	\section[Groupes sym\'etriques $S_n$]{Groupes sym\'etriques $S_n$, section 13 dans le livre de r\'ef\'erence, p.136}
	\begin{rappel}
	...
	\chapter[Homomorphismes]{Homomorphismes, chapitre 8 dans le livre de r\'ef\'erence, p.187}
	\begin{defin}
	...
	\section[Groupes quotients]{Groupes quotients, section 6 dans le livre de r\'ef\'erence, p.215}
	...
	\section[Th\'eor\`eme de Cayley]{Th\'eor\`eme de Cayley, section 3 dans le livre de r\'ef\'erence, p.196}
	...
	\begin{exem}
		$E = \{\text{graphes \'etiquet\'es \`a trois sommets}\} = \{a,b,c,d,e,f,g,h\}$, $G = S_3 = \{e, \begin{psmallmatrix}
			1&2
		...
		\end{psmallmatrix}, \begin{psmallmatrix}
			1&3&2
		\end{psmallmatrix}\}$. $G$ agit sur $E$ par permutation des sommets.

		$E^e = E$, $\card{E^e} = \card{E} = 2^3 = 8$.

		$E^{\begin{psmallmatrix}
				1&2
		\end{psmallmatrix}} = \{a, b, g, h\}$.

		$E^{\begin{psmallmatrix}
				1&2&3
		\end{psmallmatrix}} = \{a, h\}$.

		$\sfrac{E}{G} = \{\text{graphes non-\'etiquet\'es \`a trois sommets}\} = \{\{a\}, \{b,c,d\}, \{e,f,g\}, \{h\}\}$.

		$\card{\sfrac{E}{G}} = 4$.

		$\dfrac{1}{\card{G}} \sum_{g \in G} \card{E^g} = \dfrac{1}{6} ( \underbrace{8}_{e} + \underbrace{3 \times 4}_{\underset{\underset{\begin{psmallmatrix}
						2&3
				\end{psmallmatrix}}{\begin{psmallmatrix}
						1&3
			\end{psmallmatrix}}}{\begin{psmallmatrix}
					1&2
		\end{psmallmatrix}}} + \underbrace{2 \times 2}_{\underset{\begin{psmallmatrix}
		1&3&2
	\end{psmallmatrix}}{\begin{psmallmatrix}
	1&2&3
\end{psmallmatrix}}} ) = \dfrac{24}{6} = 4$.
		\begin{figure}[h]
			\centering
			\begin{tikzpicture}
				\coordinate (1) at (60:1);
				\coordinate (2) at (0,0);
				\coordinate (3) at (0:1);
				\fill (1) circle (2pt) node[above] {$1$};
				\fill (2) circle (2pt) node[below] {$2$};
				\fill (3) circle (2pt) node[below] {$3$};
			...
			\begin{tikzpicture}
				...
				\draw (1) -- (2) -- (3) -- cycle;
			\end{tikzpicture}
			\caption{Repr\'esentation de $E$}
		\end{figure}
	\end{exem}

	\chapter{Compl\'ements/rappels}
	Le signe d'une permutation $\sigma$:
	\begin{equation*}
		\signe(\sigma) = \prod_{i < j} \dfrac{\sigma(i) - \sigma(j)}{i-j}
	\end{equation*}
	$\begin{array}{rcl}
		\signe:S_n&\to&\{-1,1\} = C_2
	\end{array}$ est un homomorphisme, donc $\signe(\sigma\eta) = \signe(\sigma)\signe(\eta)$.
	\begin{prop}
		Toute permutation se d\'ecompose en un produit de cycles disjoints.
	\end{prop}
	\begin{prop}
		Tout cycle se d\'ecompose en produit de transpositions.
		\[
		\begin{pmatrix}
			a_1&a_2&a_3&\dotsb&a_l
		\end{pmatrix} = \begin{pmatrix}
		...
		\end{pmatrix} \begin{pmatrix}
			a_1&a_{l-2}
		\end{pmatrix} \dotsb \begin{pmatrix}
			a_1&a_2
		\end{pmatrix}
		\]
	\end{prop}

	Ainsi, tout $\sigma \in S_n$ se d\'ecompose en produit de transpositions.
	\begin{prop}
		$\signe\begin{psmallmatrix}
			1&2
		\end{psmallmatrix} = -1$, avec $\begin{psmallmatrix}
			1&2
		\end{psmallmatrix} \in S_n$.
		\begin{proof}~

			\begin{align*}
				\signe\begin{psmallmatrix}
					1&2
				\end{psmallmatrix}&= \prod_{i < j} \dfrac{\begin{psmallmatrix}
						1&2
					\end{psmallmatrix}(i) - \begin{psmallmatrix}
						1&2
				\end{psmallmatrix}(j)}{i-j}\\
			&= \dfrac{(12)(1) - (12)(2)}{1-2} \dfrac{(12)(1) - (12)(3)}{1-3} \dfrac{(12)(1) - (12)(4)}{1-4} \dotsb \dfrac{(12)(2) - (12)(3)}{2-3} \dotsb \dfrac{(12)(n-1) - (12)(n)}{(n-1)-n}\\
			&= \dfrac{2-1}{1-2} \cdot \prod \dfrac{\sigma(i) - \sigma(j)}{i-j}\\
			\intertext{avec $i<j$ et $\sigma(i)<\sigma(j)$, donc le produit vaut $1$}
			&= -1
			\end{align*}
		\end{proof}
	\end{prop}

	De plus, $\signe(\eta\sigma\eta^{-1}) = \signe(\eta) \signe(\sigma) \signe(\eta)^{-1} = \signe(\sigma)$.

	$\signe\begin{psmallmatrix}
		1&j
	\end{psmallmatrix} = \signe \left(\begin{psmallmatrix}
		2&j
	\end{psmallmatrix} \begin{psmallmatrix}
		1&2
	\end{psmallmatrix} \begin{psmallmatrix}
		2&j
	\end{psmallmatrix} \right) = \signe\begin{psmallmatrix}
		1&2
	\end{psmallmatrix} = -1$.
	...
	\end{psmallmatrix} \begin{psmallmatrix}
		1&i
	\end{psmallmatrix} \right) = -1$.

	Ainsi, si $\sigma$ se d\'ecompose en $k$ transpositions, alors $\signe(\sigma) = (-1)^k$.
	\begin{exem}~%TODO dessins

		Un jouet compos\'e de deux roulettes avec les chiffres de $1$ \`a $9$. Les deux roulettes peuvent tourner dans n'importe quel sens. Il y a toujours un chiffre affect\'e par les deux roulettes en m\^eme temps.

		Est-il possible de jouer \`a ce jeu et d'obtenir l'\'etat suivant?
		\begin{figure}[h]
			\centering
			\begin{tikzpicture}[scale=.7]
				\coordinate (c1) at (0,0);
				\coordinate (c2) at (0:2);
				\coordinate (1) at (72:1);
				\coordinate (2) at (0:1);
				\coordinate (3) at (-72:1);
				\coordinate (4) at (-144:1);
				\coordinate (5) at (144:1);
				\begin{scope}[xshift=2cm]
					\coordinate (6) at (108:1);
					\coordinate (7) at (36:1);
					\coordinate (8) at (-108:1);
					\coordinate (9) at (-36:1);
				\end{scope}
				\draw (c1) circle (1.25);
				\draw (c1) circle (.75);
				\draw (c2) circle (1.25);
			...
			\begin{tikzpicture}[scale=.7]
				...
				\begin{scope}[xshift=2cm]
					\coordinate (6) at (108:1);
					\coordinate (7) at (36:1);
					\coordinate (8) at (-36:1);
					\coordinate (9) at (-108:1);
				\end{scope}
				\draw (c1) circle (1.25);
				\draw (c1) circle (.75);
				\draw (c2) circle (1.25);
				...
				\node () at (\a) {$\a$};
			\end{tikzpicture}
			\caption{Repr\'esentation du jouet dans l'\'etat initial et l'\'etat final d\'esir\'e}
		\end{figure}

		On peut mod\'eliser ce jouet \`a l'aide de permutations. Soit
		\[
		\left\langle \underbrace{\begin{psmallmatrix}
			1&2&3&4&5
		\end{psmallmatrix}}_{\alpha}, \underbrace{\begin{psmallmatrix}
			2&6&7&9&8
		\end{psmallmatrix}}_{\beta} \right\rangle \lte S_9
		\]
		le plus petit sous-groupe de $S_9$ qui contient $\alpha$ et $\beta$, $\{\varepsilon, \alpha, \beta, \alpha^{-1}, \beta^{-1}, \alpha^2, \beta^2, \dotsb\}$.

		On a $\signe(\alpha) = \signe(\beta) = 1$. Alors, toutes les positions du jeu sont obtenues en appliquant une permutation paire \`a $\{1,2,3,4,5,6,7,8,9\}$, mais $\begin{psmallmatrix}
			8&9
		\end{psmallmatrix}$ est impaire.

		Ainsi, l'\'etat d\'esir\'e ne peut pas \^etre obtenu.
	\end{exem}

	
	\section{Rappels finaux}
	\renewcommand{\arraystretch}{1.2}
	\begin{tabularx}{.9\textwidth}[c]{>{\raggedright\arraybackslash}X|>{\raggedright\arraybackslash}X|>{\raggedright\arraybackslash}X}
		\multicolumn{1}{c|}{Objet}&\multicolumn{1}{c|}{Notation}&\multicolumn{1}{c}{Exemple}\\
		\hline\hline
		1. Ensemble&$\in$, ``\'el\'ement''&$A = \{a,b,c\}$, $a \in A$\\
		\hline
		\quad sous-ensemble&$B \subseteq A \Leftrightarrow \forall a \in B, a \in A$&$B = \{a,c\}$, $B \subseteq A$\\
		\hline
		\quad fonction

		\qquad image, injective, surjective, bijective, inverse, composition, produit cart\'esien&$\begin{array}{rcl}
			f:A&\to&B\\
			a&\mapsto&f(a)
		\end{array}$&$A=\entiers$, $B=\rationels$, $\begin{array}{rcl}
		f:\entiers&\to&\rationels\\
		n&\mapsto&\sfrac{n}{2}
		\end{array}$\\
		\hline\hline
		2. Groupe: ensemble et op\'eration $(G,*)$

		\quad(associativit\'e), $\exists$neutre, $\exists$inverse&op\'eration: $*,\times,+,\circ$

		neutre: $e$

		inverse: $a^{-1}$&$\entiers, \rationels, \reels, \mathbb{C}$ avec addition,

		$\rationels_*, \reels_*, \mathbb{C}_*$ avec multiplication,

		$\mathbb{D}_3, \mathbb{D}_4, \dots, \mathbb{D}_n$ les isom\'etries d'un $n$-gone r\'egulier avec la composition,

		$S(E) = \{f:E \to E \mid \text{bijective}\}$ avec la composition,

		$S_n$ les permutations de $n$ \'el\'ements avec la composition,

		$A_n$ les permutations paires ($\signe=1$) de $S_n$ avec la composition,

		$GL(n,\reels)$ matrices inversibles $n \times n$,

		$SL(n,\reels)$ matrices $n \times n$ de $\det=1$,

		$\{e\}$ groupe trivial,

		$\entiers_2 = \{\overline0,\overline1\}$ avec addition,

		${\entiers_p}_*$ avec multiplication et $p$ premier\\
		\hline
		\quad sous-groupe

		\qquad normal/distingu\'e&$\lte$&\\
		\hline
		\quad homomorphisme

		\qquad $\ker$, $\image$&$f:G \to H$, $f(ab)=f(a)f(b)$&\\
		\hline\hline
		3. $G$-ensemble (action de $G$ sur $E$)
		...
		\qquad $(ab) \bullet x=a \bullet (b \bullet x)$&$g \bullet x$ action de $g \in G$ sur $x \in E$&$S_n$ agit sur $\{1,2,\dots,n\}$

		$\reels_*$ agit sur $\reels^n$ par multiplication par scalaire

		$G$ agit sur $G$ de plusieurs fa\c cons

		\quad $a \bullet b = ab$

		\quad $a \bullet b = aba^{-1}$

		\quad $a \bullet b = ba^{-1}$
	\end{tabularx}
	\renewcommand{\arraystretch}{1}

	Pour montrer qu'une fonction est ``bien d\'efinie'', il faut montrer que $f$ associe \`a chaque \'el\'ement de son domaine un unique \'el\'ement de son codomaine.

	Si le domaine a des classes d'\'equivalence (une \'ecriture non unique), on doit v\'erifier que $\overline{a}=\overline{b} \Rightarrow f(\overline{a})=f(\overline{b})$.
	\begin{exem}~

		\begin{proof}[Montrons que]
			$\begin{array}{rcl}
				f:\entiers_5&\to&\entiers_{10}\\
				\overline{n}&\mapsto&\widetilde{2n}
			\end{array}$ est bien d\'efinie.

			Soient $n_1, n_2 \in \entiers_5$ t.q. $\overline{n_1} = \overline{n_2}$. Alors
			\begin{align*}
				n_1-n_2&= 5k\\
				2n_1-2n_2&= 2 \cdot 5k\\
				&= 10k\\
				\widetilde{2n_1}&= \widetilde{2n_2}
			\end{align*}
		\end{proof}
	\end{exem}

	Pour montrer que $G$ est un groupe, il faut v\'erifier les trois axiomes de groupes
	\begin{itemize}
		\item[(A)]
		\item[(N)] Posons $e= \alpha \in G$. V\'erifions que $e$ est neutre pour $*$.
		\item[(I)] Soit $a \in G$. V\'erifions que $b=\beta \in G$ est l'inverse de $a$.
		\item[(C)] si ab\'elien. Faire avant (N) et (I) pour sauver du temps.
	\end{itemize}

	Pour montrer que $H$ est un sous-groupe de $G$,
	\begin{nlist}
		\item $H \neq \emptyset$. En v\'erifiant que $e_G \in H$.
		\item \begin{nlist}
			\item V\'erifier que $\forall a,b \in H$, $ab^{-1} \in H$.
			\item V\'erifier que $\forall a \in H$, $a^{-1} \in H$ et v\'erifier que $\forall a,b \in H$, $ab \in H$.
		\end{nlist}
	\end{nlist}
	\begin{exem}~
		...
		\begin{proof}[m.q.]
			$\left\{ \begin{psmallmatrix}
				a&0\\0&b
			\end{psmallmatrix} ~\middle|~ a,b \in \reels_* \right\}$ est un sous-groupe de $GL(2,\reels)$.
			\begin{ulist}
				\item $\begin{psmallmatrix}
					1&0\\0&1
				\end{psmallmatrix} \in H$, en prenant $a=b=1$.
				\item en utilisant la m\'ethode (a)

				Soient $A,B \in H$. Alors $A = \begin{pmatrix}
					a&0\\0&b
				...
				\begin{align*}
					AB^{-1}&= \begin{pmatrix}
						a&0\\0&b
					\end{pmatrix} \begin{pmatrix}
						\sfrac{1}{c}&0\\0&\sfrac{b}{d}
					\end{pmatrix}\\
					&= \begin{pmatrix}
						\sfrac{a}{c}&0\\0&\sfrac{b}{d}
					\end{pmatrix}
				\end{align*}
	...
	Pour montrer que $f:G \to H$ est un morphisme de groupes, il faut montrer que $f(ab)=f(a)f(b)$, en s'assurant d'utiliser la bonne op\'eration.

	Pour montrer que $N \trianglelefteq G$ (sous-groupe normal),
	\begin{nlist}
		\item $\forall a \in G$, $\forall b \in N$, $aba^{-1} \in N$.
		\item $\forall a \in G$, $aN=Na$.
	\end{nlist}
	\begin{exem}~

		\begin{proof}[m.q.]
			$N = \left\{ \begin{psmallmatrix}
				1&a\\0&1
			\end{psmallmatrix} ~\middle|~ a \in \reels \right\} \trianglelefteq G = \left\{ \begin{psmallmatrix}
				a&b\\0&c
			\end{psmallmatrix} ~\middle|~ a,c \neq 0 \right\}$ en supposant que $N \lte G$.

			Soient $A \in G$ et $B \in N$. Alors, $A = \begin{psmallmatrix}
				a&b\\0&c
			\end{psmallmatrix}$ et $B = \begin{psmallmatrix}
				1&d\\0&1
			\end{psmallmatrix}$. On a
			\begin{align*}
				ABA^{-1}&= \begin{pmatrix}
					a&b\\0&c
				\end{pmatrix} \begin{pmatrix}
					1&d\\0&1
				\end{pmatrix} \begin{pmatrix}
					\sfrac{1}{a}&\sfrac{-b}{ac}\\0&\sfrac{1}{c}
				\end{pmatrix}\\
				&= \begin{pmatrix}
					a&ad+b\\0&c
				\end{pmatrix} \begin{pmatrix}
	...
	\underline{Th\'eor\`eme d'isomorphisme}:

	Avec $f$ un homomorphisme et $\bar{f}$ un isomorphisme, $\bar{f}(\ker(f)a) = f(a)$
	\begin{center}
	...
	\underline{Classes \`a droite modulo $H \lte G$}:

	$a \sim b \Leftrightarrow ab^{-1} \in H$ une relation d'\'equivalence dont les classes sont $Ha$.

	$Ha=Hb \Leftrightarrow a \sim b \Leftrightarrow ab^{-1} \in H$.

	\underline{Th\'eor\`eme de Lagrange}:

	$\card{H}$ divise $\card{G}$.

	\underline{Groupes quotients}:

	Si $N \trianglelefteq G$, $\sfrac{G}{N} = \{Na \mid a \in G\}$ est un groupe avec $(Na)(Nb)=N(ab)$.

	\underline{Petit th\'eor\`eme de Fermat}:

	$G = {\entiers_p}_*$, $H = \langle \overline{m} \rangle$.
	\[
	(\overline{m})^{p-1} \equiv \overline1 \pmod{p}
	\]
	autrement dit, si $m$ n'est pas un multiple de $p$, alors $m^{p-1} \equiv 1 \pmod{p}$.

	\underline{Groupe cyclique/monog\`ene}:

	$G = \langle a \rangle = \{a^n \mid n \in \entiers\}$.
	\begin{thm}
		$G \cong \entiers$ ou $G \cong \entiers_m$, avec $m=o(a)$.
	\end{thm}

	$G$ un groupe que agit sur $E$:

	$x \in E$.

	$\stab(x) = \{a \in G \mid a \bullet x=x\} \lte G$.

	$G \bullet x = \{a \bullet x \in E \mid a \in G\} \subseteq E$.

	Les orbites forment une partition de $E$. ($x \sim y \Leftrightarrow \exists a \in G \mid x=a \bullet y$)

	\underline{Th\'eor\`eme orbite-stabilisateur}:

	$\card{G} = \card{\stab(x)} \cdot \card{G \bullet x}$.
\end{document}

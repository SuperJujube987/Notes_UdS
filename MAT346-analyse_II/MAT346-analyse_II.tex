...
\newpage
	...
	\begin{rema}[Utilit\'e]~
		...
		\begin{enumerate}[label=\alph*)]
			\item $\Gamma(\alpha, \gamma)$: $f(x) = \dfrac{\lambda e^{-\lambda x} (\lambda x)^{\alpha-1}}{\Gamma(\alpha)}$, $x \gte 0$;
			...
			Ainsi, $T_1 \sim \operatorname{exp}(\lambda)$.

			De m\^eme, si $T_i$ d\'esigne le temps entre le $(i-1)$\textsuperscript{\`eme} d\'ec\`es et le $i$\textsuperscript{\`eme} d\'ec\`es, on peut montrer que $\{T_i\}$ sont iid $\operatorname{exp}(\lambda)$.
